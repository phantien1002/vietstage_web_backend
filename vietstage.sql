...
--       mini_game_re-- =========================================================
-- VIETSTAGE POSTGRESQL SCRIPT v2.2
-- Nâng cấp từ v2.1 — bỏ instructor_requests, thay bằng Admin tạo trực tiếp
-- Thay đổi chính so với v2.0:
--   [A] Bỏ bảng learner_feedback (không có cơ sở trong proposal gốc)
--   [B] Bỏ bảng instructor_requests (không có cơ sở trong proposal/code)
--       Admin tạo Instructor trực tiếp qua POST /api/admin/create-instructor
--   [C] Bổ sung seed data đầy đủ luồng nghiệp vụ:
--       practice_sessions, practice_attempts, instructor_feedback,
--       learner_progress, learner_achievements, learner_cosmetics,
...
-- =========================================================
-- 19. Không còn bảng instructor_requests
--     Thay bằng: Admin tạo tài khoản Instructor trực tiếp
--     qua POST /api/admin/create-instructor
--     (bám sát AdminController.java + InstructorServiceImpl.java)
--     Hệ thống tự sinh password ngẫu nhiên 9 ký tự → gửi email cho Instructor
--     Instructor dùng POST /api/auth/first-time-change-password để đổi lần đầu
-- =========================================================
...
sults, point_transactions, learner_daily_challenges
--   [D] learner_profiles: bổ sung current_streak/longest_streak/last_practice_date
--       cho seed data, khớp với achievement "Week Warrior" và point_transactions
--       nguồn STREAK_BONUS
-- =========================================================
